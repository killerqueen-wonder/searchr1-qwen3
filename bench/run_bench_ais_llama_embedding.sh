...
# ----------------- 模型路径与名称配置 -----------------
# 1. Embedding 模型配置 (替换为 Llama-Nemotron 本地路径)
export EMBEDDING_MODEL_PATH="/F00120250029/lixiang_share/panghuaiwen_share/legal_R1/model/models--nvidia--llama-embed-nemotron-8b"
# 注意：MODEL_NAME 必须包含 "embedding"，以触发 shared_agent 专属线路
export MODEL_NAME="llama-nemotron-embedding-0522"

# 2. 裁判与总结模型配置 (依旧使用 Qwen3 作为下游裁判和总结)
...
# 接口与并发配置 (全新端口段，杜绝冲突)
RETRIEVER_PORT=8215
RETRIEVE_PATH="http://127.0.0.1:${RETRIEVER_PORT}/retrieve" 
SUMMARY_VLLM_PORT=8218
JUDGE_VLLM_PORT=8219
...
# ================= 3. 启动后台服务 =================
info "========================================================="
info " 🦙 Llama-Nemotron Embedding 专线评测启动"
info "========================================================="

# --- 3.1 启动纯 Llama-Nemotron 检索器 (GPU 3) ---
kill_session "retriever_llama_8215"
tmux new-session -d -s retriever_llama_8215 -n retriever

tmux_send_commands "retriever_llama_8215" \
...
info "等待所有服务拉起..."
wait_for_port $RETRIEVER_PORT $PORT_TIMEOUT "retriever_llama_8215"
wait_for_port $SUMMARY_VLLM_PORT $PORT_TIMEOUT "vllm_summary_llama"
wait_for_port $JUDGE_VLLM_PORT $PORT_TIMEOUT "vllm_judge_llama"
...
info " -> 1. 推理阶段 (Llama Embedding + Summary)"
USE_DIRECT_API="true" EMBEDDING_PIPELINE="true" python ${BASE_DIR}/searchr1-qwen3/bench/ucl/ucl_infer.py \
...
info " -> 1. 推理阶段 (Llama Embedding + Summary)"
USE_DIRECT_API="true" EMBEDDING_PIPELINE="true" python ${BASE_DIR}/searchr1-qwen3/bench/lawbench/lawbench_infer.py \
...
for folder_id in {1..6}; do
    ...
    for (( j=1; j<=$max_file; j++ )); do
        ...
        if [ -f "$FILE_PATH" ]; then
            USE_DIRECT_API="true" EMBEDDING_PIPELINE="true" python ${BASE_DIR}/searchr1-qwen3/bench/lexeval/lexeval_infer.py \
                --f_path "$FILE_PATH" \
                --model_name "${MODEL_NAME}" \
...
# ================= 7. 收尾清理 =================
info "========================================================="
info "🎉 Llama-Nemotron Embedding 专线评测任务圆满结束！"
info "========================================================="