...
export MODEL_NAME="qwen3-8b-AR-fix-turn-0712"
...
# ================= 3. 启动后台服务 =================
info "========================================================="
info " 🚀 固定轮次 LawBench 对比实验启动 (适配 4xRTX 5090 模式)"
info " 工作目录: ${BASE_DIR}"
info "========================================================="
...
# ================= 4. LawBench Fixed-Turn Experiments =================
info "================== LawBench Fixed-Turn Experiments =================="
...
for FIX_TURN in "${FIX_TURNS[@]}"; do
    EXP_MODEL_NAME="${MODEL_NAME}_fix${FIX_TURN}turn"
    LAWBENCH_PRED_DIR="${BASE_DIR}/lawbench/test/prediction/zero_shot/${EXP_MODEL_NAME}"
    LAWBENCH_SCORE_DIR="${BASE_DIR}/lawbench/test/result/${EXP_MODEL_NAME}_scored"
    LAWBENCH_RESULT_PATH="${BASE_DIR}/dataset/result/bench_result/lawbench/${EXP_MODEL_NAME}_lawbench.json"
    RESULT_PATHS+=("${LAWBENCH_RESULT_PATH}")

    info "================== LawBench fixed_turn=${FIX_TURN} =================="
    info " -> 1. 推理阶段"
    python ${BASE_DIR}/searchr1-qwen3/bench/lawbench/lawbench_infer.py \
        ...
        --max_turn 12 --fixed_turn "${FIX_TURN}" --topk 10 --workers ${WORKERS} --retriever
...
# ================= 5. 收尾 =================
info "========================================================="
info "🎉 固定轮次 LawBench 对比实验结束！报告已生成于："
