...
if __name__ == '__main__':
    ...
    prompt = (
        ...
        "思考：每次获得新信息后，你必须首先在 <think> 和 </think> 标签内进行推理。\n"
        "检索：推理过程中，如果你发现涉及某些法律条文，请把法条关键词放在 <search> 和 </search> 标签之间，调用搜索引擎，以确认该法律条文。例如：<search> 民法典 盗窃罪 </search>。"
        "系统将返回最相关的搜索结果，并置于 <information> 和 </information> 标签之间。\n"
        "再次思考：搜索结果是否有助于解决问题？如果没有帮助，则回到上一步，修改关键词再次搜索。你可以根据需要进行多次搜索，直到检索到最适合的法条。\n"
        "回答：如果你认为无需进一步获取外部知识，即可直接在 <answer> 和 </answer> 标签内提供答案，无需详细说明。例如：<answer> 北京 </answer>。\n"
