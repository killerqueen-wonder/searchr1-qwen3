...
class AsyncHybridFilterRetriever(HybridRetriever):
    ...
    async def _async_llm_filter(self, num: int, query: str, candidates: List[Dict], scores: List[float], 
                                context: str = None) -> Tuple[List[Dict], List[float]]:
        """Ablation: skip LLM rerank/noise filtering and keep the original top-N candidates."""
        if not candidates:
            return [], []
        keep_num = num if num is not None else self.topk
        return candidates[:keep_num], scores[:keep_num]
    ...
    async def async_search(self, query: str, num: int = None, return_score: bool = False, context: str = None):
        ...
        # 2. 消融实验：跳过 LLM 精排/噪音过滤，直接保留原始排序的前 num 条
...
if __name__ == "__main__":
    ...
    uvicorn.run("async_retrieval_server_without_llm_rerank:app", host="0.0.0.0", port=global_config.port, workers=1)
